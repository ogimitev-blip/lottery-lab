...
hero('Ticket & System Generator','Choose the game, spending envelope and mode. Production, benchmark, shadow and exact-system challenger modes remain separate.')
...
if mode.get('kind') in ('exact','exact_curated'):
    st.info(
        ...
        "not a statistically proven replacement for the production architecture."
    )
...
if mode.get('kind')=='exact':
    with st.expander('Exact-system position mapping'):
        st.caption('Highest model-ranked numbers are assigned to the most frequently used published system positions. This reproduces the primary EXPOSURE_AWARE mapping used in v10.')
        mp=state['position_mapping'].copy()
