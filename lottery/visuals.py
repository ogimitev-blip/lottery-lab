...
def draw_map(draws,window,backtest=None,show_model=True,show_repeat=False):
    shown=draws[:window]; z=np.zeros((len(shown),42),int); hover=np.empty((len(shown),42),object)
    ...
    for i,d in enumerate(shown):
        ...
        for n in range(1,43):
            ...
            if r is not None:
                parts.append(f'K28 capture: {int(r.pool_hits)}/6')
                if n in reps: parts.append('Repeat-rule addition')
                if n in actual and n not in pool: parts.append('Outside historical K28')
            hover[i,n-1]='<br>'.join(parts)
    cs=[[0,'#0B1627'],[.24,'#0B1627'],[.25,'#22C55E'],[.49,'#22C55E'],[.50,'#F97316'],[.74,'#F97316'],[.75,'#67E8F9'],[1,'#67E8F9']]
    y=['Latest' if i==0 else f'{i} ago' for i in range(len(shown))]
    fig=go.Figure(go.Heatmap(z=z,x=list(range(1,43)),y=y,zmin=0,zmax=3,colorscale=cs,showscale=False,text=hover,hovertemplate='%{text}<extra></extra>',xgap=1,ygap=1))
    ...
    fig.update_layout(title='Draw map — green=model hit · orange=model miss · cyan=repeat addition',height=height)
    return clean(fig)

def frequency_grid(draws,window):
    shown=draws[:min(window,len(draws))]; counts={n:sum(n in d for d in shown) for n in range(1,43)}
    z=np.zeros((6,7)); text=np.empty((6,7),object); hover=np.empty((6,7),object)
    for n in range(1,43):
        r=(n-1)//7;c=(n-1)%7;z[r,c]=counts[n];text[r,c]=str(n);hover[r,c]=f'Number {n}<br>{counts[n]} appearances in last {len(shown)} draws'
...
def gap_chart(draws):
    gp={n:next((i for i,d in enumerate(draws) if n in d),len(draws))+1 for n in range(1,43)}
    ...
    fig.add_vline(x=20,line_dash='dash',annotation_text='overdue threshold = 20');fig.update_layout(title='Current gaps',height=760)
...
def rolling_frequency(draws,numbers,window):
    ...
    for n in numbers:
        a=pd.Series([int(n in d) for d in chron]);roll=a.rolling(window,min_periods=1).mean()*100
        fig.add_trace(go.Scatter(x=x,y=roll,mode='lines',name=str(n)))
...
def pair_matrix(draws,window):
    shown=draws[:min(window,len(draws))];m=np.zeros((42,42),int)
    for d in shown:
        vals=sorted(set(map(int,d)))
        for a in vals:
            for b in vals:
                if a!=b:m[a-1,b-1]+=1
    fig=go.Figure(go.Heatmap(z=m,x=list(range(1,43)),y=list(range(1,43)),colorscale='Viridis',hovertemplate='Pair %{x}–%{y}: %{z}<extra></extra>'))
...
def performance_timeline(bt):
    ...
    fig.add_trace(go.Scatter(x=x,y=d.pool_hits,mode='lines+markers',name='K28 pool hits'))
    fig.add_trace(go.Scatter(x=x,y=d.best_ticket_hits,mode='lines+markers',name='Best ticket hits'))
